...
public class LogExplorer extends JFrame {
	...
	private HashMap<String, ArrayList<DiscreteLogEvent>> eventsMap;
	public static TrainingVizContainerFrame trainingViz;
		...
		if (file.exists()) {
			...
			for (String key : this.eventsMap.keySet()) {
				if (key.toLowerCase().contains("model_num")) {
					...
					LogExplorer.trainingViz = new TrainingVizContainerFrame(key, this.eventsMap.get(key), grade);
